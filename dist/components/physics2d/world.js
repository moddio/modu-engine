 ...
 */
import { FP_ONE, FP_HALF, toFixed, fpMul, fpDiv, fpAbs } from '../../math/fixed';
import { aabb2DOverlap, Shape2DType } from './shapes';
import { BodyType2D, vec2Zero, vec2Add, vec2Scale, vec2LengthSq, createBody2D, setBody2DIdCounter, getBody2DIdCounter } from './rigid-body';
import { computeAABB2D, detectCollision2D, resolveCollision2D } from './collision';
import { shouldCollide } from './layers';
// ============================================
// Constants
// ============================================
const GRAVITY_2D = { x: 0, y: toFixed(-30) }; // -30 units/s² (down in Y)
...
export function createWorld2D(dt = 1 / 60) {
    const world = {
        bodies: [],
...
export function addBody2D(world, body) {
    world.bodies.push(body);
}
export function removeBody2D(world, body) {
...
// World Step
// ============================================
export function stepWorld2D(world) {
    ...
    // Collision detection - single pass to detect all contacts
    ...
    // Integrate positions
    for (const body of bodies) {
        if (body.type === BodyType2D.Static)
            continue;
        if (body.isSleeping)
            continue;
 ...
 */
export function saveWorldState2D(world) {
    return {
 ...
 */
export function saveWorldStateWithHash2D(world) {
    const bodies = [];
    let hash = 0;
...
function createBodyFromState(bs) {
    ...
    if (bs.bodyType === BodyType2D.Dynamic && bs.mass > 0) {
        if (shape.type === Shape2DType.Circle) {
            ...
            const FP_ONE_TWELFTH = 5461;
            body.inertia = fpMul(fpMul(bs.mass, FP_ONE_TWELFTH), (fpMul(w, w) + fpMul(h, h)));
        }
