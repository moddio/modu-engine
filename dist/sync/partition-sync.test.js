...
import { computeStateDelta, getPartition, deserializePartition, assemblePartitions, isDeltaEmpty, getDeltaSize } from './state-delta';
import { computePartitionAssignment, getClientPartitions } from './partition';
// ============================================================================
// Test Helpers
// ============================================================================
/**
...
class SimulatedClient {
     ...
     */
    computeAssignment(clientIds, frame, reliability) {
 ...
 */
function createSnapshot(entities, frame = 0) {
...
describe('Multi-Client Partition Sync', () => {
    ...
    describe('Partition Assignment Consistency', () => {
        test('all clients compute identical partition assignments', () => {
            ...
            for (const id of clientIds) {
                ...
                client.setState(createSnapshot(Array.from({ length: entityCount }, (_, i) => ({ eid: i, type: 'Entity' })), frame));
            ...
            for (let i = 1; i < assignments.length; i++) {
                ...
                for (let p = 0; p < baseline.numPartitions; p++) {
                    expect(assignments[i].partitionSenders.get(p)).toEqual(baseline.partitionSenders.get(p));
                }
            }
        });
    ...
    describe('Partition Assembly', () => {
        test('assembled partitions produce identical delta to original', () => {
            ...
            for (const client of clients) {
                ...
                const partitionData = client.generatePartitionData(client.computeDelta(), myPartitions, assignment.numPartitions);
            ...
            expect(assembledDelta.created.map(e => e.eid)).toEqual(originalDelta.created.map(e => e.eid));
    ...
    describe('Edge Cases', () => {
        ...
        test('handles single client assignment', () => {
            ...
            client.setState(createSnapshot(Array.from({ length: 100 }, (_, i) => ({ eid: i, type: 'Entity' })), 0));
