...
interface Physics2DLike {
  ...
  onCollision(
    typeA: string,
    typeB: string,
    handler: (a: Entity, b: Entity) => void,
  ): this;
...
export interface ConnectOptions {
  ...
  joinToken?: string;
  /** Application ID for the network central service. If not provided, reads from window.__MODU_APP_ID__ or defaults to 'dev'. */
  appId?: string;
}
...
export class Game {
  ...
  private divergenceHistory: Array<{
    frame: number;
    field: string;
    local: any;
    server: any;
  ...
  private collisionHandlers: Map<string, (a: Entity, b: Entity) => void> =
    new Map();
  ...
  start(callbacks: GameCallbacks = {}): void {
    ...
    const localClientId =
      "local-" + Math.random().toString(36).substring(2, 10);
    ...
    console.log(
      "[ecs] Started in local/offline mode with clientId:",
      localClientId,
    );
  }
  ...
  async connect(
    ...
    if (callbacksOrOptions) {
      ...
      const isConnectOptions =
        "nodeUrl" in callbacksOrOptions ||
    ...
    if (wasStartedLocally) {
      ...
      console.log(
        `[ecs] Connecting to room "${roomId}"... (local game will be replaced with server state)`,
      );
    } else {
      // ONLINE-FIRST MODE: connect() called without start()
      // Start game locally while waiting for server connection
      console.log(
        `[ecs] Starting game locally, connecting to room "${roomId}" in background...`,
      );
    ...
    const network: NetworkSDK | undefined =
      typeof window !== "undefined" ? (window as any).moduNetwork : undefined;
    if (!network) {
    ...
    try {
      this.connection = await network.connect(roomId, {
        ...
        appId:
          connectOptions.appId ||
          (typeof window !== "undefined" && (window as any).__MODU_APP_ID__) ||
          "dev",
      ...
      const connectDuration =
        (typeof performance !== "undefined" ? performance.now() : Date.now()) -
        connectStartTime;
      console.log(
        `[ecs] Connected successfully in ${connectDuration.toFixed(0)}ms, clientId: ${this.connection.clientId}`,
      );
      ...
      if ("onReliabilityUpdate" in this.connection) {
        this.connection.onReliabilityUpdate = (
          scores: Record<string, number>,
          version: number,
        ) => {
      ...
      if ("onResyncSnapshot" in this.connection) {
        this.connection.onResyncSnapshot = (
          data: Uint8Array,
          frame: number,
          inputs: ServerInput[],
        ) => {
      ...
      const connectDuration =
        ...
        connectStartTime;
      console.error(
        `[ecs] Connection failed after ${connectDuration.toFixed(0)}ms:`,
        err?.message || err,
      );
      console.error(
        "[ecs] Make sure the server is running. Check: 1) Central service on port 9001, 2) Node server",
      );
  ...
  private handleMajorityHash(frame: number, majorityHash: number): void {
    ...
    if (localHash === undefined) {
      ...
      if (frame % 100 === 0) {
        console.warn(
          `[state-sync] No local hash for frame ${frame} (history has ${this.stateHashHistory.size} frames)`,
        );
      }
      return;
    }
    ...
    if (localHash === majorityHash) {
      ...
      if (!this.resyncPending) {
        ...
        console.error(
          `  Local hash:    ${localHash.toString(16).padStart(8, "0")}`,
        );
        console.error(
          `  Majority hash: ${majorityHash.toString(16).padStart(8, "0")}`,
        );
        ...
        if (this.connection?.requestResync) {
          ...
          console.warn(
            `[state-sync] Cannot request resync - SDK does not support requestResync()`,
          );
        }
      }
    }
  ...
  private handleResyncSnapshot(
    ...
    console.warn(
      `[ENGINE-RESYNC] Received resync snapshot (${data.length} bytes) for frame ${serverFrame}. currentFrame=${this.currentFrame} isDesynced=${this.isDesynced} inputs=${inputs.length}`,
    );
    ...
    if (!snapshot) {
      console.error(
        `[state-sync] Failed to decode resync snapshot - no snapshot data (tried binary and JSON)`,
      );
    ...
    console.error(
      `  Local hash at desync:    ${this.desyncLocalHash.toString(16).padStart(8, "0")}`,
    );
    console.error(
      `  Majority hash at desync: ${this.desyncMajorityHash.toString(16).padStart(8, "0")}`,
    );
    ...
    if (inputs.length > 0) {
      ...
      if (ticksToRun > 0) {
        console.log(
          `[RESYNC-CATCHUP] Running ${ticksToRun} ticks with ${inputs.length} inputs (snapshotFrame=${snapshotFrame}, serverFrame=${serverFrame})`,
        );
    ...
    console.log(
      `[RESYNC-DEBUG] After load: ${allEnts.length} total, ${syncedEnts.length} synced, ${localEnts.length} local (camera etc). Hash=${newLocalHash.toString(16)}`,
    );
    if (localEnts.length > 0) {
      console.log(
        `[RESYNC-DEBUG] Local entities: ${localEnts.map((e) => e.type + "#" + e.eid.toString(16)).join(", ")}`,
      );
    }

    if (serverHash && newLocalHash === serverHash) {
      console.log(
        `[state-sync] Hard recovery successful - hash=${newLocalHash.toString(16).padStart(8, "0")}`,
      );
    } else if (!serverHash) {
      // No server hash in snapshot - this is OK, we can't verify but state was loaded
      console.log(
        `[state-sync] Hard recovery completed - hash=${newLocalHash.toString(16).padStart(8, "0")}`,
      );
    } else {
      console.error(
        `[state-sync] Hard recovery hash mismatch: expected=${serverHash?.toString(16).padStart(8, "0")} got=${newLocalHash.toString(16).padStart(8, "0")}`,
      );
    }
  ...
  private logDesyncDiff(serverSnapshot: any, serverFrame: number): void {
    ...
    const syncPercent =
      totalFields > 0 ? (matchingFields / totalFields) * 100 : 100;
    const syncColor =
      syncPercent > 95
        ? "color: #4ade80"
        : syncPercent > 50
    ...
    console.group(
      `%cState Comparison (${diffs.length} differences)`,
    ...
    if (diffs.length > 0) {
      const fieldDiffs = diffs.filter(
        (d) => d.server !== "MISSING" && d.local !== "MISSING",
      );
      ...
      if (fieldDiffs.length > 0) {
        ...
        const tableData = fieldDiffs.map((d) => ({
          ...
          server: d.server,
          delta:
            typeof d.local === "number" && typeof d.server === "number"
      ...
      if (localOnly.length > 0) {
        console.groupCollapsed(
          `%cLocal Only - ${localOnly.length} entities not on server`,
          "color: #fbbf24",
        );
      ...
      if (serverOnly.length > 0) {
        console.groupCollapsed(
          `%cServer Only - ${serverOnly.length} entities not on local`,
          "color: #fbbf24",
        );
      ...
      console.log(
        "%cNo field differences found (hash mismatch may be due to RNG or string state)",
    ...
    if (recentInputCount > 0) {
      ...
      const recentData = this.recentInputs
        .slice(-recentInputCount)
        .map((input) => ({
          frame: input.frame,
          client: input.clientId.slice(0, 8),
          move: input.data?.move
            ? `(${input.data.move.x},${input.data.move.y})`
            : "-",
          shoot: input.data?.shoot ? "yes" : "-",
          target: input.data?.target
            ? `(${Math.round(input.data.target.x)},${Math.round(input.data.target.y)})`
            : "-",
        }));
  ...
  private handleConnect(
    ...
    if (DEBUG_NETWORK) {
      ...
      console.log(
        `[ecs] Snapshot:`,
        snapshot
          ? { frame: snapshot.frame, entityCount: snapshot.entities?.length }
          : "none",
      );
    ...
    if (hasValidSnapshot) {
      ...
      for (const input of inputs) {
        ...
        if (
          inputClientId &&
          (data?.type === "join" || data?.type === "reconnect")
        ) {
      ...
      if (expectedHash !== undefined && loadedHash !== expectedHash) {
        console.error(
          `[SNAPSHOT] HASH MISMATCH! loaded=0x${loadedHash.toString(16)} expected=0x${expectedHash?.toString(16)}`,
        );
      }
      ...
      if (ticksToRun > MAX_CATCHUP_FRAMES) {
        console.warn(
          `[CATCHUP] Too many frames to catch up (${ticksToRun} > ${MAX_CATCHUP_FRAMES}). Requesting fresh snapshot.`,
        );
      ...
      if (this.localRoomCreated) {
        console.log(
          "[ecs] Local-first handoff: flushing local state, recreating with server identity",
        );
  ...
  private handleTick(
    ...
    if (frame <= this.lastProcessedFrame) {
      if (DEBUG_NETWORK) {
        console.log(
          `[ecs] Skipping old frame ${frame} (already at ${this.lastProcessedFrame})`,
        );
      }
      return;
    }
    ...
    if (DEBUG_NETWORK && inputs.length > 0) {
      ...
      console.log(
        `[ecs] onTick frame=${frame}: ${inputs.length} inputs (${types})`,
      );
    }
    ...
    if (majorityHash !== undefined && majorityHash !== 0) {
      ...
      console.warn(
        `[state-sync] No majorityHash in tick ${frame} (expected with ${this.activeClients.length} clients)`,
      );
    }
  ...
  private sendStateSync(frame: number): void {
    ...
    if (
      this.activeClients.length > 1 &&
      this.connection.clientId &&
      this.connection.sendPartitionData &&
      this.prevSnapshot
    ) {
      ...
      if (frame % 60 === 0 && !isDeltaEmpty(delta)) {
        console.log(
          `[delta] frame=${frame} created=${delta.created.length} deleted=${delta.deleted.length} bytes=${deltaSize}`,
        );
      }
      ...
      if (!isDeltaEmpty(delta)) {
        ...
        const numPartitions = computePartitionCount(
          entityCount,
        ...
        const myPartitions = getClientPartitions(
          assignment,
          this.connection.clientId,
        );
        ...
        for (const partitionId of myPartitions) {
          ...
          if (hasChangesInPartition) {
            const partitionData = getPartition(
              delta,
              partitionId,
              numPartitions,
            );
  ...
  private routeInputToEntity(clientId: string, data: any): void {
    ...
    if (DEBUG_NETWORK) {
      ...
      console.log(
        `[ecs] routeInput: clientId=${clientId.slice(0, 8)}, numId=${numId}, entity=${entity?.eid || "null"}, data=${JSON.stringify(data)}`,
      );
    }
  }
  ...
  private loadNetworkSnapshot(snapshot: any): void {
    if (DEBUG_NETWORK) {
      console.log(
        `[ecs] Loading snapshot: ${snapshot.entities?.length} entities`,
      );
    }
    ...
    if (snapshot.clientIdMap) {
      const snapshotMappings = Object.entries(snapshot.clientIdMap.toNum) as [
        string,
        number,
      ][];
      ...
      this.clientIdToNum = new Map(
        snapshotMappings.map(([k, v]) => [k, v as number]),
      );
      this.numToClientId = new Map(
        snapshotMappings.map(([k, v]) => [v as number, k]),
      );
    ...
    for (const entity of this.world.query(Player)) {
      ...
      if (player.clientId === 0) {
        ...
        console.error(
          `[ecs] Player entity ${entity.eid} has clientId=0 (invalid)`,
        );
        continue;
      }
      ...
      if (clientIdStr) {
        ...
        if (network?.registerClientId) {
          ...
          if (DEBUG_NETWORK) {
            console.log(
              `[ecs] Registered clientId ${clientIdStr.slice(0, 8)} from snapshot entity`,
            );
          }
        }
        ...
        if (DEBUG_NETWORK) {
          console.log(
            `[ecs] Snapshot has entity for client ${clientIdStr.slice(0, 8)}`,
          );
        }
      }
    }
    ...
    if (snapshot.inputState) {
      ...
      if (snapshotKeys !== loadedKeys) {
        console.error(
          `[INPUT-STATE] KEYS DIFFER! snapshot=[${snapshotKeys}] loaded=[${loadedKeys}]`,
        );
      }
    }
  }
      ...
      if (serverSnapshot) {
        ...
        if (this.currentFrame === serverSnapshot.frame) {
          ...
          if (localHash !== serverHash) {
            console.warn(
              `[ecs] DRIFT detected at frame ${serverSnapshot.frame}: local=${localHash}, server=${serverHash}`,
            );
          }
        } else {
  ...
  private compareSnapshotFields(serverSnapshot: any): void {
    ...
    if (wasSync && !isSync && !this.divergenceCaptured) {
      ...
      const inputsInRange = this.recentInputs.filter(
        (i) => i.frame > lastGoodFrame && i.frame <= frame,
      );
    ...
    if (
      diffs.length > 0 &&
      newPercent < 100 &&
      this.divergenceCaptured &&
      frame % 60 === 0
    ) {
      console.warn(
        `[DIVERGENCE] Frame ${frame}: still diverged (${newPercent.toFixed(1)}% sync, first at ${this.firstDivergenceFrame})`,
      );
    }
  }
  ...
  private showDivergenceDiff(
    ...
    for (const entity of this.world.getAllEntities()) {
      if (entity.has(Player)) {
        ...
        if (ownerClientId) {
          entityOwners.set(
            entity.eid,
            clientLabels.get(ownerClientId) || ownerClientId.slice(0, 8),
          );
        }
      }
    }
    ...
    lines.push(
      `Frame: ${frame} | Last good: ${lastGoodFrame} | Authority: ${this.checkIsAuthority()}`,
    );
    lines.push(
      `Clients: ${clientList.map((cid) => `${clientLabels.get(cid)}=${cid.slice(0, 8)}`).join(", ")}`,
    );
    lines.push(``);
    ...
    for (const d of diffs) {
      ...
      lines.push(
        `  ${d.entity}#${d.eid.toString(16)}${ownerStr}.${d.comp}.${d.field}: local=${d.local} server=${d.server}${delta}`,
      );
    }
    lines.push(``);
    ...
    for (const input of inputs) {
      const label =
        clientLabels.get(input.clientId) || input.clientId.slice(0, 8);
      lines.push(`  f${input.frame} [${label}]: ${JSON.stringify(input.data)}`);
    }
    lines.push(``);

    if (this.lastGoodSnapshot) {
      const goodEnts = Object.keys(
        this.lastGoodSnapshot.snapshot.entities || {},
      ).length;
      lines.push(
        `LAST GOOD SNAPSHOT (f${lastGoodFrame}): ${goodEnts} entities`,
      );
    } else {
    ...
    if (this.lastServerSnapshot.decoded) {
      const serverEnts = Object.keys(
        this.lastServerSnapshot.decoded.entities || {},
      ).length;
      lines.push(
        `SERVER SNAPSHOT (f${this.lastServerSnapshot.frame}): ${serverEnts} entities`,
      );
    }
  ...
  checkIsAuthority(): boolean {
    ...
    const minLen = Math.min(
      this.localClientIdStr.length,
      this.authorityClientId.length,
    );
    return (
      this.localClientIdStr.substring(0, minLen) ===
      this.authorityClientId.substring(0, minLen)
    );
  }
