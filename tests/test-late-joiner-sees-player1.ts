/**
 * Test: Late Joiner Sees Player 1
 *
 * This test reproduces the bug: "Player 2 doesn't see Player 1's character"
 *
 * The scenario:
 * 1. Client 1 creates a room
 * 2. Client 2 joins shortly after (before any snapshot is sent)
 * 3. Client 2 should receive Client 1's join input in INITIAL_STATE
 * 4. Client 2 should spawn Player 1's character
 *
 * DEBUGGING: This test logs the exact data received to diagnose why
 * Player 2 might not be seeing Player 1.
 */

import WebSocket from 'ws';

const CENTRAL_URL = process.env.CENTRAL_URL || 'http://localhost:9001';
const ROOM_ID = 'late-joiner-test-' + Date.now();
const APP_ID = 'dev';

console.log('=== Late Joiner Sees Player 1 Test ===');
      ...
      if (msgType === MSG_TICK) {
        ...
        if (parsed && parsed.inputs.length > 0) {
          for (const input of parsed.inputs) {
            if (input.data?.type === 'join' || input.data?.type === 'reconnect') {
              client.joinInputsReceived.push(input);
              console.log(`[${name}] TICK contained join input: seq=${input.seq}, clientId=${input.data?.clientId}`);
            }
          }
        }
      }
...
async function runTest() {
  ...
  try {
    ...
    // Wait a tiny bit - this simulates the race condition
    // where Player 2 joins before Player 1 has sent any snapshot
    await wait(100);

    // Phase 2: Client B joins
    console.log('\n=== Phase 2: Client B joins (late joiner) ===');
    ...
    for (const input of clientB.initialStateInputs) {
      ...
      console.log(`    seq=${input.seq}, type=${dataType}, clientId=${dataClientId}`);
    }
    ...
    // Test 1: Client B should have received join input for Player A
    ...
    check('Client B received Player A join input', hasPlayerAJoin,
      `Got ${clientB.joinInputsReceived.length} join inputs: ${JSON.stringify(clientB.joinInputsReceived.map(i => i.data?.user?.id || i.data?.clientId))}`);

    // Test 2: The join input should be in INITIAL_STATE (not just a later TICK)
    ...
    // Test 3: If snapshot is empty (no physics2d), Client B should still get Player A's join
    if (!clientB.initialStateSnapshot?.physics2d) {
      console.log('\n  NOTE: Snapshot was empty (no physics2d). This is expected if joining quickly.');
      console.log('  In this case, the join input is CRITICAL for spawning Player A.');
      check('Empty snapshot but join input present', hasPlayerAJoin,
        'Without snapshot AND without join input, Player A would be invisible');
    }
    ...
    if (failed > 0) {
      console.log('\n!!! BUG REPRODUCED !!!');
      console.log('Player 2 is NOT receiving Player 1 join input correctly.');
      process.exit(1);
    } else {
